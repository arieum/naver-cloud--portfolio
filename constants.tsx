...
export const SELF_INTRO = [
  {
    question: "1. 자기자신을 자유롭게 소개해 주세요.",
    answer: "저는 춘천에 위치한 네이버 데이터센터를 보며, 서비스 뒤에서 보이지 않게 반복되는 운영과 관리가 얼마나 중요한 일인지를 처음 실감했습니다. 수많은 서비스가 끊김 없이 동작하기 위해서는 문제가 발생한 뒤 대응하는 방식이 아니라, 문제가 반복되지 않도록 구조를 설계하는 일이 필요하다고 느꼈습니다. 이 문제의식이 제가 네이버클라우드에 관심을 갖게 된 출발점입니다. 저는 기술이 어떤 반복 업무를 대신하고 어떤 판단을 자동화할 수 있는지를 고민해 온 개발자입니다. IBM AI 해커톤에서 발표 준비 서비스를 기획·개발하며 ‘요약→대본→리허설→모의평가’를 하나의 고정된 워크플로우로 설계했습니다. 발표 발화와 핵심 키워드 매칭 과정에서 단순 문자열 비교의 한계를 실험으로 확인했고, 임베딩 기반 유사도 비교로 전환해 키워드 인식 정확도를 62%에서 96%까지 개선했습니다. 이를 통해 사용자의 발화가 자연스러워도 의도를 안정적으로 인식하는 흐름을 만들 수 있었습니다. 포스코 STS304 불량 예측 프로젝트에서는 불량률 약 2.7%의 극단적인 클래스 불균형으로 F1-score가 20% 수준에 정체되는 문제가 있었습니다. 단순 오버샘플링을 넘어, 불량 데이터를 군집화한 뒤 관리 단위별로 SMOTE를 적용하는 방식을 선택했고, 그 결과 F1-score를 40% 이상으로 개선했습니다. 복잡한 문제를 그대로 두지 않고, 자동화와 개선이 가능한 구조로 재정의한 경험이었습니다. 이러한 경험들을 통해 저는 ‘왜 이 문제가 반복되는지’를 먼저 정의하고, 그 답을 워크플로우와 구조로 고정하는 개발자로 성장해 왔습니다. 네이버클라우드에서 AI가 반복적인 운영 판단을 보조하고, 사람이 더 중요한 의사결정에 집중할 수 있도록 돕는 서비스 기획에 기여하고 싶습니다."
  },
  {
    question: "2. AI를 기반으로 자동화 또는 기준 대비 성과를 올린 경험(프로젝트)이 있으면 서술하고, 그 경험에서 자신이 어떤 역할을 했었고, 결과로 어떤 지식이나 교훈을 얻었는지 설명해주세요.",
    answer: "AI가 단순히 결과를 예측하는 데서 끝나지 않고, 사용자의 판단과 행동까지 자동으로 이어질 수 있는 구조를 만들고자 한 경험이 있습니다. 포스코 AI 프로젝트로 진행한 ‘POPIPOPI’는 실시간 통화 내용을 분석해 보이스피싱 위험을 탐지하고, 근거 제시와 대응 가이드까지 자동으로 연결하는 서비스입니다. 저는 이 프로젝트에서 AI 파이프라인 설계와 자동화 흐름 정의를 담당했습니다. 기존 보이스피싱 탐지 모델은 위험 여부를 확률로만 제시해, 사용자가 왜 의심되는지 이해하기 어렵다는 한계가 있었습니다. 이를 개선하기 위해 음성을 STT로 변환한 뒤 문장 단위로 정상/의심 분류와 위험도를 산출하고, 동시에 법령·기관 문서에서 근거를 검색해 설명까지 생성하는 자동화 구조를 설계했습니다. 특히 LLM의 환각 문제를 줄이기 위해 RAG 방식을 적용하고, 검색 결과를 리랭킹해 핵심 근거만 사용하도록 기준을 명확히 했습니다. 이로써 ‘탐지 → 근거 → 대응’이 끊기지 않는 자동 흐름을 만들 수 있었습니다. 또한 모바일 환경에서의 실사용을 고려해 경량화 실험을 병행했습니다. 기존 모델 대비 추론 지연과 메모리 사용량을 기준으로 KD, 양자화, QLoRA 등을 비교했고, 성능 저하를 최소화하면서도 저지연 조건을 만족하는 설정을 도출했습니다. 이를 통해 단순 탐지 정확도 비교를 넘어, 운영 환경 기준에서 성능을 평가하고 개선하는 경험을 할 수 있었습니다. 이 프로젝트를 통해 저는 AI 자동화에서 중요한 것은 모델 성능 자체보다, 어떤 판단을 어디까지 자동화하고 어떤 근거를 함께 제공할 것인가에 대한 기준 설정임을 배웠습니다. 이후에는 AI를 '결과 생성 도구'가 아니라, 반복되는 판단과 대응을 구조적으로 대신하는 시스템으로 바라보게 되었고, 이러한 관점은 클라우드 AI 운영 자동화 서비스를 기획하는 데에도 중요한 기반이 될 것이라 생각합니다."
  },
  {
    question: "3. 팀을 이뤄 협업을 하면서 성과를 이뤘던 경험을 적어주세요. 자신이 어떤 역할로 팀에 기여했고 어떤 교훈을 얻었는지를 포함해 주세요. ",
    answer: "저는 팀 프로젝트에서 기술적 논의가 감정이나 피로에 묻히지 않도록, 문제를 근거와 실험으로 정리하는 역할을 맡아왔습니다. 포스코 AI·빅데이터 아카데미에서 진행한 STS304 M형 결함 예측 프로젝트와 IBM AI 해커톤 프로젝트에서, 저는 기술적 대안을 구조화하고 팀이 납득할 수 있는 판단 기준을 제시하며 협업 성과를 이끌었습니다. STS304 M형 결함 예측 프로젝트에서는 제출 직전, 불량 원인 분석 결과를 설명하는 그래프가 결론을 충분히 뒷받침하지 못한다는 문제를 발견했습니다. 연이은 새벽 회의로 팀원들이 지쳐 있었지만, 저는 각 그래프가 유도하는 해석과 논리의 단절 지점을 근거와 함께 설명하며 수정 방향을 제안했습니다. “이미 분석은 잘 되어 있으니, 이 부분만 보완하면 완성도가 크게 높아진다”는 메시지로 설득한 결과, 팀원들이 수정에 동의했고 핵심 변수 중심의 재시각화와 설명 보완을 통해 분석의 설득력을 강화할 수 있었습니다. 그 결과 해당 프로젝트는 아카데미 내 분석상과 프로젝트 최우수상을 수상했습니다. 이후 참여한 IBM AI 해커톤에서 발표 준비 도우미(PitchMate)를 개발할 때도 기술적 의견 차이가 있었습니다. 발표 리허설 기능에서 단순 문자열 비교 방식을 사용하자는 의견이 있었지만, 저는 이 방식이 맥락 유사성을 반영하지 못한다는 한계를 지적했습니다. 예시 문장을 통해 문제를 공유한 뒤, 발화를 임베딩 벡터로 변환해 키워드와의 유사도를 비교하는 방식을 제안했고, 두 접근을 직접 실험해 결과를 비교했습니다. 그 결과 키워드 매칭 정확도가 62%에서 96%로 개선되는 것이 확인되었고, 실험 결과를 근거로 팀의 합의를 이끌어 최종 구현 방향으로 채택할 수 있었습니다. 이 프로젝트는 IBM AI 해커톤에서 최우수상을 수상했습니다. 이 경험들을 통해 저는 협업에서 중요한 것은 빠른 결론이 아니라, 팀 모두가 납득할 수 있는 판단 근거를 만드는 일임을 배웠고, 이후에도 이러한 방식으로 팀 성과에 기여해 왔습니다."
  }
];
